...
pub use stateful_tree::StatefulTree;
...
impl Node {
    pub fn new(to_show: String, path: String) -> Self {
        Node {
            to_show: to_show,
            path: path,
        }
...
impl TreeItem {
    pub fn new_leaf(node: Node) -> Self {
        Self {
            text: node,
            style: Style::default(),
            children: Vec::new(),
    ...
    pub fn path(&self) -> String {
        self.text.path.clone()
    }

    pub fn new<Children>(node: Node, children: Children) -> Self
    where
        Children: Into<Vec<TreeItem>>,
    {
        Self {
            text: node,
            style: Style::default(),
    ...
    pub fn height(&self) -> usize {
        Into::<Text>::into(self.text.to_show.clone()).height()
    }
...
impl<'a> StatefulWidget for Tree<'a> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
        ...
        for item in visible.iter().skip(state.offset).take(end - start) {
            ...
            for (j, line) in Into::<Text>::into(item.item.text.to_show.clone())
                .lines
                .iter()
                .enumerate()
            {
